...
def upscale(filename, image_path, output_image_dir):
    ...
    with torch.inference_mode():
        ...
        cliptextencode_6 = cliptextencode.encode(
            text="person playing chess, by bnha Gerard Garouste, 2019 an oil painting, oil on canvas. The painting features a person standing next to a table, holding a checkered box. A door is open in the background. There are deep and expressive shadows. Clear face, sharp eyes",
            clip=get_value_at_index(loraloader_10, 1),
        )

        cliptextencode_24 = cliptextencode.encode(
            text="blurry, smooth", clip=get_value_at_index(loraloader_10, 1)
        )
        ...
        save_tensors_as_images(get_value_at_index(imageupscalewithmodel_43, 0), output_image_dir, file_prefix="huge-retex")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Upscalings script.")
    parser.add_argument('--image_path', type=str, required=True, help='Input image path')
    parser.add_argument('--output_image_dir', type=str, default="test/", help='Input image path')
